...
impl PyMLContext {
    ...
    fn create_device_tensor(
        ...
        use super::tensor::PyMLDeviceTensor;
        use rustnn::executors::onnx::OrtDeviceTensor;
        use rustnn::tensor::{DeviceKind, DeviceTensorHandle};
    ...
    fn compute_onnx(
        ...
        for input_id in &graph.graph_info.input_operands {
            ...
            let dtype_str = match data_type {
                rustnn::graph::DataType::Int4 => "int8",
                rustnn::graph::DataType::Uint4 => "uint8",
            ...
            let tensor_data = match data_type {
                rustnn::graph::DataType::Float32 => {
                    let data: Vec<f32> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Float32(data)
                }
                rustnn::graph::DataType::Float16 => {
                    ...
                    let data_u16: Vec<u16> = data_f32
                        ...
                        .collect();
                    rustnn::executors::onnx::TensorData::Float16(data_u16)
                }
                rustnn::graph::DataType::Int8 => {
                    let data: Vec<i8> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Int8(data)
                }
                rustnn::graph::DataType::Uint8 => {
                    let data: Vec<u8> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Uint8(data)
                }
                rustnn::graph::DataType::Int32 => {
                    let data: Vec<i32> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Int32(data)
                }
                rustnn::graph::DataType::Uint32 => {
                    let data: Vec<u32> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Uint32(data)
                }
                rustnn::graph::DataType::Int4 => {
                    let data: Vec<i8> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Int8(data)
                }
                rustnn::graph::DataType::Uint4 => {
                    let data: Vec<u8> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Uint8(data)
                }
                rustnn::graph::DataType::Int64 => {
                    let data: Vec<i64> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Int64(data)
                }
                rustnn::graph::DataType::Uint64 => {
                    let data: Vec<u64> = flat.call_method0("tolist")?.extract()?;
                    rustnn::executors::onnx::TensorData::Uint64(data)
                }
            };
